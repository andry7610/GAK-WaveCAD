...
История:
  ...
  v0.2 — фикс: float() на числовых параметрах из config
  v0.3 — k_thermal из конфига, формула напряжения для сферы, профили как списки
"""
...
class ThermalMonitor(BaseModule):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        # Читаем k_thermal (как передаёт app.py), с откатом на k
        self.k = float(cfg.get("k_thermal", kwargs.get("k_thermal",
                  cfg.get("k", kwargs.get("k", 0.6)))))
        self.alpha = float(cfg.get("alpha", kwargs.get("alpha", 70e-6)))
        self.E = float(cfg.get("E", kwargs.get("E", 2.0e9)))
        self.nu = float(cfg.get("nu", kwargs.get("nu", 0.33)))
        self.T_inner = float(cfg.get("T_inner", kwargs.get("T_inner", 310.0)))
        self.T_outer = float(cfg.get("T_outer", kwargs.get("T_outer", 293.0)))
        self.n_layers = int(cfg.get("n_layers", kwargs.get("n_layers", 20)))
    ...
    def run(self) -> bool:
        ...
        # Тепловое напряжение для сферической оболочки (деление на 2)
        sigma_thermal = self.E * self.alpha * dT / (2 * (1 - self.nu))
        grad_T = dT / self.h
        ...
        self.results = {
            ...
            'grad_T': float(grad_T),
            'Q': float(Q),
            'sigma_thermal_Pa': float(sigma_thermal),
            'sigma_thermal_MPa': float(sigma_thermal) / 1e6,
            'r_profile': r.tolist(),
