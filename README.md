...
**v0.3** — модульная система моделирования физических процессов в сферической оболочке.
...
│   ├── test_integration.py
│   └── test_plasma_monitor.py   # 49 плазменных тестов
├── .github/workflows/
...
- 6 отдельных тестовых скриптов (`run_*.py` в корне)
- pytest: 49 плазменных + 1 интеграционный = **50 passed**
- Интеграционный тест проверяет полную цепочку: осмос → термалка → акустика → коллапс → магноны → EM → кросс-связи
...
# Интеграционный тест через pytest
...
**Что готово:**
- 7 физических модулей реализованы и связаны в единую цепочку
- Кросс-связи между всеми модулями
- CI на GitHub Actions — полностью зелёный
- 50 тестов: 49 плазменных + 1 интеграционный
- Единая архитектура: `init() → run() → get_results() → print_report()`

**В планах:**
- Веб-интерфейс / дашборд с графиками
