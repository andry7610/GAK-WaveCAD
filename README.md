...
- **6 отдельных тестовых скриптов** (`run_*.py` в корне репозитория) — каждый проверяет свой физический модуль отдельно
- **pytest: 50 passed** — 49 плазменных тестов (`test_plasma_monitor.py`) + 1 интеграционный (`test_integration.py`)
- **Интеграционный тест** прогоняет полную цепочку: осмос → термалка → акустика → коллапс → магноны → EM → кросс-связи, проверяя передачу данных между модулями, ненулевые кросс-связи, корректность индекса стабильности (0..1) и системного статуса
...
# Локальный запуск интеграционного теста
...
**Демо-версия.** Все 7 модулей реализованы и связаны в единую цепочку. CI проходит полностью. Дальнейшие планы: веб-дашборд с графиками, YAML-конфиг с параметрами по умолчанию, расширенная документация API.
